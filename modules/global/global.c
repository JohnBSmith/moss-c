...
int mf_mul_dec(mt_object* x, mt_object* a, mt_object* b){
    ...
    mpy:
    if(binary_op_method(x,a,b,a,3,"mul")){
        if(op_method_error) return 1;
    }else{
        return 0;
    }
    rmpy:
    if(binary_op_method(x,a,b,b,4,"rmul")){
        if(op_method_error) return 1;
    }else{
        return 0;
    }
...
int mf_mod_dec(mt_object* x, mt_object* a, mt_object* b){
    ...
    case mv_string:{
        ...
        case mv_list:{
            ...
            s = mf_format(template,list->size,list->a);
            break;
        }
        default:
            s = mf_format(template,1,b);
        }
...
static
int is_table2(mt_object* a, mt_table* t1, mt_table* t2){
    if(a->type==mv_table){
        mt_table* p = (mt_table*)a->value.p;
        return p==t1 || p==t2;
    }else{
        return 0;
    }
...
int mf_isin(mt_object* x, mt_object* a, mt_object* b){
    ...
    case mv_int:
        ...
        return 0;
    case mv_long:
        x->value.b = is_table(b,mv_type_long);
        x->type = mv_bool;
        return 0;
    case mv_float:
    ...
    case mv_complex:
        ...
        return 0;
    case mv_string:
        x->value.b = is_table2(b,mv_type_string,mv_type_iterable);
        x->type = mv_bool;
        return 0;
    case mv_bstring:
        ...
        return 0;
    case mv_list:
        x->value.b = is_table2(b,mv_type_list,mv_type_iterable);
        x->type = mv_bool;
        return 0;
    case mv_function:
        ...
        return 0;
    case mv_range:
        x->value.b = is_table2(b,mv_type_range,mv_type_iterable);
        x->type = mv_bool;
        return 0;
    case mv_map:
        x->value.b = is_table2(b,mv_type_dict,mv_type_iterable);
        x->type = mv_bool;
        return 0;
    case mv_table:{
...
void mf_delete_load_tab(){
    int i;
    for(i=0; i<sizeof(load_tab)/sizeof(mt_load_tab_element); i++){
        if(load_tab[i].t!=NULL){
            mf_table_dec_refcount(load_tab[i].t);
        }
    }
}
...
static
int mf_load(mt_object* x, mt_object* m){
    if(m->type!=mv_string){
        mf_type_error1("in load(id): id (type: %s) is not a string.",x);
        return 1;
    }
    mt_string* s = (mt_string*)m->value.p;
    mt_bstr id;
...
static
int mf_use(mt_object* module, mt_map* d){
    mt_map* m;
    if(module->type==mv_table){
        mt_table* t = (mt_table*) module->value.p;
        m = t->m;
    }else if(module->type==mv_function){
        mt_function* f = (mt_function*) module->value.p;
        m = f->m;
    }else{
        mf_type_error("type error in use: module is not of type table.");
        return 1;
    }
...
int mf_fimport(mt_object* x, int argc, mt_object* v){
    ...
    for(i=0; i<n; i++){
        if(a[i].taken==2){
            ...
            int e;
            if(a[i].value.type==mv_string){
                e = mf_load(&module,&a[i].value);
                id = (mt_string*)a[i].key.value.p;
                id->refcount++;
            }else{
                e = mf_load(&module,&a[i].key);
            ...
            mf_map_set_str(mv_gvtab,id,&module);
            module.value.p->refcount--;
            mf_str_dec_refcount(id);
        }
    }
    if(argc==2 && module.type!=mv_null){
...
#define PUT_STACK_SIZE 10
mt_basic* put_stack[PUT_STACK_SIZE];
int put_stack_sp = 0;

static
int found_in_put_stack(void* p){
    int i;
    for(i=0; i<put_stack_sp; i++){
        if(put_stack[i]==p) return 1;
    }
    return 0;
}
...
int mf_put(mt_object* x){
    ...
    case mv_list:{
        mt_list* list = (mt_list*)x->value.p;
        if(found_in_put_stack(list)){
            printf("[...]");
            break;
        }
        printf("[");
        int e,cond;
        cond = put_stack_sp < PUT_STACK_SIZE;
        if(cond){
            put_stack[put_stack_sp] = (mt_basic*)list;
            put_stack_sp++;
        }
        e = mf_buffer_print(list->size,list->a);
        if(cond) put_stack_sp--;
        if(e){
            mf_traceback("put");
            return 1;    
        }
...
int mf_fassertion_failed(mt_object* x, int argc, mt_object* v){
    if(argc!=1){
        mf_argc_error(argc,1,1,"assertion_failed");
        return 1;
    }
    if(v[1].type!=mv_string){
        mf_type_error1("in assertion_failed(s): s (type: %s) is not a string.",&v[1]);
        return 1;
    }
