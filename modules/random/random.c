...
static uint32_t kx=123456789, ky=234567891;
static uint32_t kz=345678912, kw=456789123, kc=0;
...
uint32_t mf_rand32(mt_rand_state* s){
  ...
  t = s->z+s->w+s->c; s->z = s->w; s->c = t>0x7fffffff; s->w = t&0x7fffffff;
  s->x += 1411392427;
  return s->x+s->y+s->w;
}
...
mt_rand_state* mf_rand_state_new(uint32_t seed){
  ...
  s->x=kx*(seed+1); s->y=ky*(seed+1); s->z=kz;
  s->w=kw; s->c=kc;
  return s;
}
