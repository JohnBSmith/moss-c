...
void bint_set_value(bint* x, int n){
  if(n<0){
    x->sgn=-1;
    n=-n;
  }else{
...
static
void bint_mpy(bint* a, bint* b, bint* y){
  bint a1; a1.a=NULL;
  bint b1; b1.a=NULL;
  bint x; x.a=NULL;
  bint_copy(a,&a1);
...
/*
Object* long_bit(Object** v, int n){
  if(n!=1 || v[0]->type!=id_long || v[1]->type!=id_int){
    printf("Error in x.bit(i).\n");
    return 0;
  }
  mt_long* x = (mt_long*)v[0];
  int i = ((Int*)v[1])->value;
  return (Object*)new_int(get_bit(x,i));
}

Object* long_set_bit(Object** v, int n){
  if(n!=2 || v[0]->type!=id_long || v[1]->type!=id_int || v[2]->type!=id_int){
    printf("Error in x.set_bit(xi,b).\n");
    return 0;
  }
  mt_long* x = (mt_long*)v[0];
  int i = ((Int*)v[1])->value;
...
mt_string* bint_to_string(bint* x){
  ...
  mt_string* s = mf_str_new(v.size,v.a);
  mf_vec_delete(&v);
  return s;
} 
...
mt_long* mf_long_mpy(mt_long* a, mt_long* b){
  ...
  // bint_mpy_karatsuba(&a->value,&b->value,&y->value);
  return y;
}
